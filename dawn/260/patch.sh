...
# original hashes
if [ "${sum}" == "af82a646e947d2ba69bf0c7f2432454a" ]; then
	reltype="os"
	echo "--- Applying for: International (OS) version"
fi
if [ "${sum}" == "688800bf397aaf1d088116d2ff267bb8" ]; then
	reltype="cn"
...
# See dev_tools/network.md (up-to-date as of 2.6.0)
if [[ "$reltype" == "os" ]]; then
	servers=$(cat <<EOF
...
if [[ "$reltype" == "cn" ]]; then
	...
	sed -i "s/log-upload-os/log-upload/g" "launcher.bat"
